...
@WebServlet("/back_end/BackWithdrawRequest/*")
public class WithdrawRequestBackServlet extends HttpServlet {
	private WithdrawRequestService withdrawSvc;
	private MembersService memSvc;
	private BalanceHistoryService balanceSvc;
	
	public void init() throws ServletException {
		withdrawSvc = new WithdrawRequestServiceImpl();
		memSvc = new MembersServiceImpl();
		balanceSvc = new BalanceHistoryServiceImpl();
	}
...
//		}
		...
		if ("getAll".equals(choice)) {
			...
			String emp=String.valueOf(session.getAttribute("empId"));
...
//		}
		...
		if ("UpdateStatus".equals(choice)) {
			...
			String amountStr = req.getParameter("amount_" + index);
			...
			try {
				...
				WithdrawRequest withdrawRequest = withdrawSvc.getWRById(wrId);
				changeValue = (Integer.valueOf(amountStr))*-1;
	            ...
	            if(reqStatus == 1) {
	            	...
	            	Integer mbrId = withdrawRequest.getMbrId();
...
//	    			balanceHistory.setBidOrderId(1);//與一般訂單2選1
	    			balanceHistory.setWrId(wrId);//從提款編號取，不一定有
	            ...
	            withdrawRequest.setEmpId(empId);// 後台取得
